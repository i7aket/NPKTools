...
public sealed class FertilizerOptimizationService : IFertilizerOptimizationService
{
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="target"/> is null.</exception>
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is signalled.</exception>
    public FertilizerSolutions FindSolutions(PpmTarget target,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(target);

        return new FertilizerSolutions(
            FindMacroSolutions(target, cancellationToken),
            FindMicroSolutions(target, cancellationToken));
    }
