...
public interface IFertilizerOptimizationService
{
    ...
    /// <param name="cancellationToken">Token used to cancel the search.</param>
    /// <returns>A tuple containing the macro and micro solution sets, each empty when none were found.</returns>
    (Solutions Macro, Solutions Micro) FindSolutions(PpmTarget target, CancellationToken cancellationToken = default);
}
